...
class spModel {
	...
	{
		$dbConfig=$GLOBALS['spConfig']['db']['0'];
		$linkfunction = 'mysqli_connect';//( TRUE == $dbConfig['persistent'] ) ? 'mysqli_pconnect' : 'mysqli_connect';
		$this->conn = $linkfunction($dbConfig['host'], $dbConfig['login'], $dbConfig['password']) or die("数据库链接错误 : " . iconv('gbk','utf-8',mysqli_error()));
		mysqli_select_db($this->conn,$dbConfig['database']) or die("无法找到数据库，请确认数据库名称正确！");
	}
	...
	{
		if( TRUE != $GLOBALS['spConfig']['db']['persistent'] )@mysqli_close($this->conn);
	}
}
