<?php
class gnclientController extends spController
{
	/**
	 * 国内客户管理
	 */
	public function clientListAc()
	{
		...
		if($_REQUEST['out']=='1'){
            $data=$obj->clientList(0,5000);
			$pubObj=new pub();
			$titleArr=array("客户姓名","客户公司名称 ","客户邮箱","客户类型","客户电话","客户在线工具","创建日期",
			"持有者","客户公司网址","客户手机","客户备注");
			
			$keyArr=array("first_name","company_name","mail","type_name","phone","tool_number","datetime"
		...
		$this->data=$data;
		$this->display('gnclient/clientList.html');
	}
	public function addClientAc()
	{
		...
		$this->display('gnclient/clientAdd.html');
	}
	function checkClientAc()
	{
		...
		$phone=$_POST['phone'];
		$sql="select * from client where company_name='$company_name' and  phone='$phone' limit 1";
		$data=$obj->getOne($sql);
		if(!empty($data)){
	...
	public function doAddClientAc()
	{
		...
		unset($row['id']);
		$param=array();
		$url=spUrl('gnclientController','addClientAc');
		if($id==''){
			$row['datetime']=date("Y-m-d H:i:s");
			$row['country']='中国';
			$row['is_pub']=0;
			if($obj->exist('client',array('company_name'=>$_POST['company_name'],'phone'=>$_POST['phone']))){
				$res['success']=false;
				$res['msg']='此客户已存在';
	...
	function clientMoreAc()
	{
		...
		$data=$obj->clientMore();
		$this->data=$data;
		$this->display('gnclient/clientMore.html');
	}
	function clientPubListAc()
	{
		$obj=new gnclient();
		$data=$obj->clientPubList();
		if($_REQUEST['out']=='1'){
			$pubObj=new pub();
			$titleArr=array("客户姓名","客户公司名称 ","客户类型","客户电话","客户在线工具","创建日期",
			"持有者","客户公司网址","客户手机","客户备注");
			
			$keyArr=array("first_name","company_name","type_name","phone","tool_number","datetime"
			,"opername","url","mobile_phone","remark");
			$arrReplaceName=array('datetime');
			$pubObj->outExcelCommon($data['data'], $titleArr, $keyArr,'',$arrReplaceName);
			exit;
		...
		$this->end_date=$_REQUEST['end_date'];
		$this->data=$data;
		$this->display('gnclient/clientPubList.html');
	}
	function toMineAc()
	{
		$db=new spModel();
		...
		$sql="update client set is_pub=0,operid='$operid' where id in($id)";
		$flag=$db->exec($sql);
		$res=$this->returnMsg($flag);
		die(json_encode($res));
	}
